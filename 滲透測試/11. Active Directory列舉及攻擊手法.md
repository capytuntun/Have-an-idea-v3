...
## Silver Tickets
>[!NOTE]
>偽造 **TGS（服務票證）**，只需要該服務帳號的密碼 hash（如 CIFS/HTTP 的 service account）
...
>[!NOTE]
>#### 範例
>- IIS 伺服器需要幫用戶去存取 SQL Server。
>- 管理員把 IIS 的服務帳號設成「僅能對 `MSSQLSvc/sql01.domain.local` 做委派」。
>- 這樣 IIS 就只能冒用用戶去連 SQL，不可能拿著用戶身份去連 File Server 或 DC。
...
>[!NOTE]
>在「資源方」的帳號物件上設定 `msDS-AllowedToActOnBehalfOfOtherIdentity`， 意思是資源（被存取的服務）自己決定「誰可以代表使用者來訪問我」。這樣可以避免原本要 Domain Admin 權限才能修改委派屬性，降低管理負擔，但同時也帶來濫用風險。
### PowerView 指令
``` powershell=
