...
| TGT (Ticket Granting Ticket)      | An Authentication ticket required to obtain tickets for specific services. |
...
```
### BloodHound 輸出結果
- **Overview** - 提供摘要資訊，例如該帳號目前有多少個有效工作階段，以及是否能接觸高價值目標。
- **Node Properties** - 顯示該 AD 帳號的資訊，例如顯示名稱與職稱。
- **Extra Properties** - 提供更詳細的 AD 資訊，例如識別名稱（distinguished name）與帳號建立時間。
- **Group Membership** - 顯示該帳號所屬的群組相關資訊。
- **Local Admin Rights** - 提供該帳號在網域內主機上擁有系統管理員權限的相關資訊。
- **Execution Rights** - 提供該帳號擁有的特殊權限資訊，例如能否透過 RDP 登入機器。
- **Outbound Control Rights** - 顯示該帳號對其他 AD 物件具有修改其屬性權限的資訊。
- **Inbound Control Rights** - 提供可修改該帳號屬性的其他 AD 物件相關資訊。
