...
```
``` powershell
Get-ChildItem -Filter *.ps1 -Recurse -EA SilentlyContinue | Select-String -Pattern "password"
```
## Config Files 
``` cmd
...
```
``` powershell
(Get-ChildItem -Filter *.ps1 -Recurse -ErrorAction SilentlyContinue).FullName
```
``` powershell
